...
export default function Home() {
  ...
  const handlerButtonClick = (value) => {
    if (value === "=") {
      ...
      setExpression((preveExpression) => preveExpression + value);
    }
  };
    ...
    <main className="flex min-h-screen  flex-col items-center p-24 ">
      ...
      <div className="bg-white p-6 rounded-1g shadow-1g">
        <input
          ...
          readOnly
        />
        <input type="text" className="w-full text-4xl font-bold mb-4 focus-outline" value={result} readOnly />
        <div className="grid grid-cols-4 gap-2 ">
          {buttons.map((btn) => (
            <button
              key={btn}
              onClick={() => handlerButtonClick(btn)}
              className=" text-4xl bg-gray-400 hover:bg-gray-400 p-2 rounded-lg "
            >
              {btn}
            </button>
          ))}
